...
/proc/reject_bad_name(t_in, allow_numbers = TRUE, max_length = MAX_NAME_LEN, ascii_only = TRUE, strict = FALSE, cap_after_symbols = TRUE, cap_at_start = TRUE) // SKYRAT EDIT CHANGE - allow_numbers to TRUE
	...
	for(var/i = 1, i <= t_len, i += length(char))
		...
		switch(text2ascii(char))
			...
			// A  .. Z
			if(65 to 90, 1040 to 1071, 1025) //Uppercase Letters ////CHANGE SpaceCub: add Cyrillic support. Original "65 to 90"
			...
			if(1024, 1026 to 1104, 1106 to INFINITY) //CHANGE SpaceCub: add Cyrillic support. Original "127 to INFINITY"
				if(ascii_only)
					if(strict)
						return
					continue
