...
/proc/reject_bad_text(text, max_length = 512, ascii_only = TRUE)
	if(ascii_only)
		if(length(text) > max_length)
			return null
		var/static/regex/non_ascii = regex(@"[^\x20-\x7E\t\n\u0400-\u04FF]") //CHANGE SpaceCub: add Cyrillic support. Original "[^\x20-\x7E\t\n"
...
/proc/reject_bad_name(t_in, allow_numbers = TRUE, max_length = MAX_NAME_LEN, ascii_only = TRUE, strict = FALSE, cap_after_symbols = TRUE, cap_at_start = TRUE) // SKYRAT EDIT CHANGE - allow_numbers to TRUE
	...
	for(var/i = 1, i <= t_len, i += length(char))
      ...
      // BUBBER EDIT BEGIN -- Allow Accented Letters & Do Not Autocapitalize After Apostrophe
			...
			if(65 to 90,192 to 214,216 to 222, 1040 to 1071, 1025) //Uppercase Letters ////CHANGE SpaceCub: add Cyrillic support. Original "65 to 90,192 to 214,216 to 222"
				number_of_alphanumeric++
				last_char_group = LETTERS_DETECTED

			// a  .. z, Latin-1 Supplement Letters
			if(97 to 122,223 to 246,248 to 255, 1072 to 1103, 1105) //Lowercase Letters //CHANGE SpaceCub: add Cyrillic support. Original "97 to 122,223 to 246,248 to 255"
			...
			if(1024, 1026 to 1104, 1106 to INFINITY) //CHANGE SpaceCub: add Cyrillic support. Original "127 to 191,215,247,256 to INFINITY"
				if(ascii_only)
					if(strict)
						return
					continue
...
/proc/sanitize_css_class_name(name)
	var/static/regex/regex = new(@"[^a-zA-Z0-9а-яА-ЯёЁ]","g")  //CHANGE SpaceCub: add Cyrillic support. Original "[^a-zA-Z0-9]"
